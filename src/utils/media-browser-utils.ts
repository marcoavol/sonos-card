...
export function renderMediaBrowserItem(item: MediaPlayerItem, showTitle = true, showMetadata = false) {
  return html`
    ...
    <div class="title" ?hidden=${!showTitle}>
      ...
      ${showMetadata
        ? html`
            <div class="metadata">
              <div class="content-id">${item.media_content_id}</div>
              <div class="content-type">${item.media_content_type}</div>
            </div>
          `
        : nothing}
    </div>
